...
:: 2map-columns ( a b quot -- c )
    [
        a columns set: ( a1 a2 a3 a4 )
        b columns set: ( b1 b2 b3 b4 )
...
TYPED:: m4. ( a: matrix4 b: matrix4 -- c: matrix4 )
    [
        ...
        b1 first  a1 n*v set: c1a
        b2 first  a1 n*v set: c2a
        b3 first  a1 n*v set: c3a
        b4 first  a1 n*v set: c4a

        b1 second a2 n*v c1a v+ set: c1b
        b2 second a2 n*v c2a v+ set: c2b
        b3 second a2 n*v c3a v+ set: c3b
        b4 second a2 n*v c4a v+ set: c4b

        b1 third  a3 n*v c1b v+ set: c1c
        b2 third  a3 n*v c2b v+ set: c2c
        b3 third  a3 n*v c3b v+ set: c3c
        b4 third  a3 n*v c4b v+ set: c4c
...
:: (rotation-matrix4) ( diagonal triangle-hi triangle-lo -- matrix )
    matrix4 (struct) set: triangle-m
    diagonal scale-matrix4 set: diagonal-m
...
TYPED:: rotation-matrix4 ( axis: float-4 theta: float -- matrix: matrix4 )
    ...
    float-4{ -1.0  1.0 -1.0 0.0 } set: triangle-sign

    c float-4-with set: cc
    s float-4-with set: ss
    1.0 float-4-with set: ones
    ones cc v- set: 1-c
    axis axis v* set: axis2

    axis2 cc ones axis2 v- v* v+ set: diagonal

    axis { 1 0 0 3 } vshuffle axis { 2 2 1 3 } vshuffle v* 1-c v*
    float-4{ t t t f } vbitand set: triangle-a
    ss axis v* triangle-sign v* set: triangle-b
    triangle-a triangle-b v+ set: triangle-lo
    triangle-a triangle-b v- set: triangle-hi
...
TYPED:: frustum-matrix4 ( xy: float-4 near: float far: float -- matrix: matrix4 )
    [
        ...
        float-4{ 0.0 -1.0 0.0 0.0 } set: negone
...
TYPED:: q>matrix4 ( q: float-4 -- matrix: matrix4 )
    ...
    !   0                      0                      0                      1
    q { 2 1 1 3 } vshuffle  q { 3 3 2 3 } vshuffle  v*  set: triangle-a
    q { 0 0 0 3 } vshuffle  q { 1 2 3 3 } vshuffle  v*  set: triangle-b

    triangle-a float-4{ 2.0 2.0 2.0 0.0 } v*  triangle-b float-4{ -2.0 2.0 -2.0 0.0 } v*
    [ v- ] [ v+ ] 2bi set: ( triangle-hi triangle-lo )

    q q v* first4 {
        [ [ + ] [ - ] [ - ] tri* ]
        [ [ - ] [ + ] [ - ] tri* ]
        [ [ - ] [ - ] [ + ] tri* ]
    } 4 ncleave 1.0 float-4-boa set: diagonal
